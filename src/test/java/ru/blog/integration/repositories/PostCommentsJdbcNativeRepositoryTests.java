...
@ActiveProfiles("test")
@DataJdbcTest
@Import(PostCommentsJdbcNativeRepository.class)
public class PostCommentsJdbcNativeRepositoryTests {
  ...
  public void shouldFindPostComment() {
    ...
    Assertions.assertEquals(postComment.getPostId(), comment.getPostId());
    Assertions.assertEquals(postComment.getUserId(), comment.getUserId());
    Assertions.assertEquals(postComment.getComment(), comment.getComment());
